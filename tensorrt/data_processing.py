...
LABEL_FILE_PATH = os.path.join('data/rzx/rzx.names')
...
class PreprocessYOLO(object):
    ...
    def _load_and_resize(self, input_image_path):
        ...
        image_resized = cv2.resize(image_raw, (608, 608), interpolation=cv2.INTER_LINEAR)
        return image_raw, image_resized
...
class PostprocessYOLO(object):
    ...
    def _process_yolo_output(self, outputs_reshaped, resolution_raw):
        ...
        for output, mask in zip(outputs_reshaped, self.masks):
            ...
            #     boxes[i].extend(box[i])
            #     categories[i].extend(category[i])
            #     confidences[i].extend(confidence[i])
            boxes.append(box)
    ...
    def _process_feats(self, output_reshaped, mask):
        ...
        # batch_size, grid_h, grid_w, _, _ = output_reshaped.shape
    ...
    def _filter_boxes(self, boxes, box_confidences, box_class_probs):
        ...
        boxes = boxes[pos]
        # batch_boxes = boxes[i][pos]
        classes = box_classes[pos]
        scores = box_class_scores[pos]
        # ret_boxes.append(batch_boxes)
        # ret_classes.append(classes)
        # ret_scores.append(classes)
        #
        # ret_boxes = np.array(ret_boxes)
        # ret_classes = np.array(ret_classes)
        # ret_scores = np.array(ret_scores)
        return boxes, classes, scores
        # return ret_boxes, ret_classes, ret_scores
